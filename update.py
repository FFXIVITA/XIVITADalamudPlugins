...
PROVIDER = os.getenv("PROVIDER", 'https://ffxivita.github.io/XIVITADalamudPlugins/dist')
...
def merge_manifests(stable, testing):
    ...
    for key in manifest_keys:
        ...
        manifest["RepoUrl"] = get_repo_url(testing_path) or get_repo_url(stable_path) or testing_manifest.get("RepoUrl", stable_manifest.get("RepoUrl")) # noqa
        manifest["AssemblyVersion"] = stable_manifest["AssemblyVersion"] if stable_manifest else testing_manifest["AssemblyVersion"] # noqa
        ...
        manifest["IconUrl"] = testing_manifest.get("IconUrl", stable_manifest.get("IconUrl", "https://avatars.githubusercontent.com/u/62541782?s=200&v=4")) # noqa
...
def generate_markdown(manifests):
    ...
    ita2 = timezone(timedelta(hours=2))
    ita1 = timezone(timedelta(hours=1))
    ...
    for manifest in manifests:
        ...
        last_updated = datetime.fromtimestamp(manifest["LastUpdated"], tz=ita1).strftime("%d-%m-%Y")
